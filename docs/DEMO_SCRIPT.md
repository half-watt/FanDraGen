...
**Env (optional):** `GEMINI_API_KEY` for summary polish (`gemini_enrichment_applied` in payload). `FANDRAGEN_LIVE_ESPN=1` for ESPN headlines/standings. `FANDRAGEN_NBA_API=1` for real stats.nba.com game logs via [`nba_api`](https://github.com/swar/nba_api). **Required data:** Kaggle-format NBA stats CSV (default path `data/kaggle/nba_player_stats_2425.csv`).
