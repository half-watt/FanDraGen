"""Player and team news: local JSON shell plus optional live ESPN headlines."""
...
class NewsTool(BaseTool):
    ...
    def fetch_player_news(self, state: WorkflowState, player_names: list[str] | None = None) -> ToolResult:
        ...
        result = ToolResult(
            ...
            data=news,
            supporting_points=[f"Found {len(news)} player news items in the local feed."]
    ...
    def fetch_team_news(self, state: WorkflowState, team_names: list[str] | None = None) -> ToolResult:
        ...
        result = ToolResult(
            ...
            data=news,
            supporting_points=[f"Found {len(news)} team news items in the local feed."]
            + ([enrichment["note"]] if enrichment else []),
